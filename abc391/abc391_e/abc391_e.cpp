...
signed main() {
    ...
    rep(i,n){
        rep(j,mypow(3,n-1-i)){
            ll x=0;
            if(b[n-i][3*j]+b[n-i][3*j+1]+b[n-i][3*j+2]>=2) x=1;
            b[n-1-i].pb(x);
            ll y=0;
            if(i==0) y=2-x-(b[n-i][3*j]+b[n-i][3*j+1]+b[n-i][3*j+2]);
            else{
                if((c[n-i][3*j]>0&&c[n-i][3*j+1]>0&&c[n-i][3*j+2]>0)||(c[n-i][3*j]<0&&c[n-i][3*j+1]<0&&c[n-i][3*j+2]<0)){
                    y=c[n-i][3*j]+c[n-i][3*j+1]+c[n-i][3*j+2]-(max(max(abs(c[n-i][3*j]),abs(c[n-i][3*j+1])),abs(c[n-i][3*j+2]))*abs(c[n-i][3*j])/c[n-i][3*j]);
                }else{
                    y=c[n-i][3*j]+c[n-i][3*j+1]+c[n-i][3*j+2]-max(max(c[n-i][3*j],c[n-i][3*j+1]),c[n-i][3*j+2])-min(min(c[n-i][3*j],c[n-i][3*j+1]),c[n-i][3*j+2]);
                }
            }
            c[n-1-i].pb(y);
        }
