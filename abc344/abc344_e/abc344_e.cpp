...
using namespace atcoder;
#endif
void process_queries(int N, list<int>& A, const vector<pair<int, pair<int, int>>>& queries) {
    unordered_map<int, list<int>::iterator> positions; // 要素とその位置のマップ
    ...
    for (auto& query : queries) {
        if (query.first == 1) {
            ...
            auto it = positions[x];
            auto new_it = A.insert(next(it), y); // xの直後にyを挿入
            positions[y] = new_it; // yの位置を更新
        } else if (query.first == 2) {
            int x = query.second.first;
            auto it = positions[x];
            positions.erase(x); // マップからxの位置を削除
            A.erase(it); // xを削除
        }
    }
}
