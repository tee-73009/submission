...
#define YesNo(T) if(T){cout<<"Yes"<<endl;}else{cout<<"No"<<endl;}
#define YESNO(T) if(T){cout<<"YES"<<endl;}else{cout<<"NO"<<endl;}
#define fin(...) ({ __VA_ARGS__; return 0; })
...
#line 54 "library/template/template.hpp"
using namespace std;
#line 2 "code.cpp"
#if __has_include(<atcoder/all>)
#include <atcoder/all>
...
long long min_subarray_sum(const vector<long long>& A) {
    ...
    for (int right = 0; right < n; ++right) {
        ...
        // If current_sum becomes positive, reset it to 0 and move left pointer
        if (current_sum > 0) {
            current_sum = 0;
            left = right + 1;
        }
...
long long max_subarray_sum(const vector<long long>& A) {
    ...
    for (int right = 0; right < n; ++right) {
        ...
        // If current_sum becomes negative, reset it to 0 and move left pointer
        if (current_sum < 0) {
            current_sum = 0;
            left = right + 1;
        }
...
signed main() {
    ...
    ll sum=bit.sum(0,n);
    if(c>0) print(max(sum+(c-1)*max_subarray_sum(a),sum));
    else print(max(sum+(c-1)*min_subarray_sum(a),sum));
}
