...
long long totalSupport(const vector<long long>& A, int x) {
    long long sum = 0;
    for (int cost : A) {
        sum += min(cost, x);
    }
...
int maxTransportationSupport(int N, long long M, const vector<long long>& A) {
    int low = 0, high = *max_element(A.begin(), A.end());
    if (totalSupport(A, high) <= M) {
        return -1;
    }
    while (low <= high) {
        int mid = low + (high - low) / 2;
        if (totalSupport(A, mid) <= M) {
            low = mid + 1;
        } else {
            high = mid - 1;
        }
...
int main() {
    ...
    input(a);
    sort(all(a));
    ll ans=maxTransportationSupport(n, m, a);
    if(ans==-1) print("infinite");
    else print(ans);
}
